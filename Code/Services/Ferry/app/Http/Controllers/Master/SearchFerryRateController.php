...
class SearchFerryRateController extends Controller
{
    ...
    public function index(){
        ...
        $users = DB::table(_FERRY_RATE_MASTER_)->get();
...
foreach ($users as $user) {
    ...
    $collection->each(function ($value) use ($JsonItem) {
            $existingRecord = DB::table(_SEARCH_FERRY_RATE_)->where('RateCode', $value)->first();
            if ($existingRecord) {
            ...
            try {
                $updateQuery = DB::table(_SEARCH_FERRY_RATE_)
    ...
    // ]);
            ...
            // ]);
                ...
                try {
                    DB::table(_SEARCH_FERRY_RATE_)->insert([
                        'RateCode' => $value,
